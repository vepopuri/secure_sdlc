...
import { RiskBadge, StatusBadge, ActionLevelBadge, SeverityBadge } from '../components/common/StatusBadge';
    ...
    <Box>
      ...
      <Paper sx={{ p: 2.5, mb: 3 }}>
        ...
        <Stack direction="row" gap={3} flexWrap="wrap" alignItems="center">
          <FormControlLabel
            control={<Switch checked={agent.status === 'enabled'} disabled={busy || !role.canRunAgents} onChange={(e) => handleToggle(e.target.checked)} />}
            label={agent.status === 'enabled' ? 'Enabled for this workspace' : 'Disabled for this workspace'}
          />
          {role.canRunAgents && (
            <Button variant="contained" size="small" disabled={agent.status === 'disabled' || busy} onClick={handleRun}>
        ...
        {runResult && (
          <Alert severity="success" sx={{ mt: 2 }} onClose={() => setRunResult(null)}>
            {runResult}
          </Alert>
        )}
      </Paper>
          ...
          <Paper sx={{ p: { xs: 2.5, md: 3 }, mb: 3 }}>
            ...
            {relatedAgents.length > 0 && (
              <DetailSection title="Related agents">
                <Stack direction="row" gap={0.5} flexWrap="wrap">
                  {relatedAgents.map((r) => (
                    <Chip key={r.id} size="small" label={r.name} variant="outlined" clickable onClick={() => navigate(`/agents/${r.id}`)} />
                  ))}
                </Stack>
              </DetailSection>
            )}
          ...
          {agent.id === 'remediation_agent' && (
            <Paper sx={{ p: { xs: 2.5, md: 3 } }}>
              <Alert severity="warning" icon={<GitHubIcon fontSize="inherit" />} sx={{ mb: 1.5 }}>
                <strong>Live integration.</strong> This agent is wired to a real system — most other
                actions in this app are simulated. The button below opens (or reuses) an actual draft
                pull request on <strong>vepopuri/secure_sdlc</strong> via the GitHub API.
              </Alert>
              {liveResult?.ok && (
